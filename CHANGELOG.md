...
* fix: `icp canister status` again falls back to the publicly readable state-tree information when the caller may not read the status. Replicas carrying the status-visibility feature reject those calls with a new error code (`IC0542`), which the fallback did not recognise, so the command failed with `Error looking up canister <id>` instead of printing the controllers and module hash. `IC0541`, returned on subnets with administrators, is now recognised too. The fallback also no longer depends on how the rejection was delivered: the replica checks status visibility both when accepting the ingress message and again during execution, so the same denial can arrive as either an uncertified or a certified reject, and only the former was matched.
