...
* feat: an environment may now list the canisters it leaves out, with `exclude-canisters:`. It works with or without `canisters:` — on its own it starts from every canister in the project, and alongside a `canisters:` selection it narrows that selection — so an environment that deploys all but one canister no longer has to enumerate the rest. An entry names a canister the way the rest of the project does (`backend`, or `services/crm:worker` for a subproject's canister), or names a whole subproject with a trailing `:` and no canister name (`services/crm:`), which also covers the subprojects nested beneath it. An entry that matches no canister in the project is rejected when the project is loaded. In a workspace, a subproject's own environment contributes its exclusions too, written relative to itself and confined to its own subtree, so a vendored project keeps the canisters out that its author kept out.
