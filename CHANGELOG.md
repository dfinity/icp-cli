...
* fix(dependencies): an environment's `canisters:` list is now honored in a workspace. Vendoring a project whose `staging` deploys only some of its canisters used to deploy *all* of them to the workspace's `staging`, because the dependency's list was dropped while its settings and init args were kept. Each project's environment block now decides **its own** canisters — the root's list no longer removes a dependency's, and a dependency that writes no list still contributes all of its canisters — so a vendored project deploys the canisters its author chose, which is what it does standalone. `canisters: []` keeps a project out of an environment entirely. A list may also name another project's canisters by their path-based name (`"vendor/openemail:frontend"`), which decides that project's canisters instead; doing so for a project that also writes its own list for the same environment is an error rather than a silent precedence. A member-scoped `icp deploy` no longer treats a sibling the environment leaves out as a dependency awaiting deployment. Three manifest errors a vendored project would be rejected for standalone are now also rejected when it is vendored: an environment naming a canister the project cannot address, a path that is not one of its dependencies, and two blocks for the same environment name.
