...
* fix(dependencies): a dependency's own environment `canisters:` selection is now honored in a workspace. Vendoring a project whose `staging` deploys only some of its canisters used to deploy *all* of them to the workspace's `staging`, because the selection was dropped while its settings and init args were kept. It now prunes the workspace environment the same way it does when the dependency is deployed on its own, including an empty selection meaning "none of mine", and a member-scoped `icp deploy` no longer treats a sibling the environment excludes as a dependency awaiting deployment. A workspace root that names its environment's canisters explicitly still gets exactly that list, matching the existing root-wins precedence for settings. Two manifest errors a vendored project would be rejected for standalone are now also rejected when it is vendored: an environment selecting a canister it does not declare, and two blocks for the same environment name.
