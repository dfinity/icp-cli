...
* feat: when Internet Identity is enabled on a launcher-managed network (not a Docker `image` network), `icp network start` registers the identities `alice`, `bob` and `charlie` with it and prints the seed index to sign in with for each, so they no longer have to be created by hand after every network reset. Set `ii-identities` on the network to choose other names, or to `[]` to register none.
