...
* fix(dependencies): an environment's `canisters:` list is now honored in a workspace — a vendored project's own list used to be dropped, so all of its canisters deployed. Each project's list decides **its own** canisters, naming another project's decides theirs, and two places disagreeing about one project's is an error, reported when that environment is used rather than when the project is read. So `canisters: []` keeps out only the project that writes it — use `exclude-canisters:` to drop a canister or a whole subproject the root does not own — and a listed environment's canisters now come in declaration order. See [Which canisters an environment holds](docs/concepts/project-dependencies.md#which-canisters-an-environment-holds).
