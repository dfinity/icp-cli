...
use crate::options::{Format, FormatOpt};

#[derive(Debug, Parser)]
pub struct ListCmd {
    #[command(flatten)]
    with_format: FormatOpt,
}
...
pub fn exec(env: &Env, cmd: ListCmd) -> Result<(), ListKeysError> {
    ...
    match cmd.with_format.format {
        ...
        Format::Human => handle_human_output(list, defaults),
    }
...
fn handle_human_output(list: IdentityList, defaults: IdentityDefaults) {
