...
/// Opens (creating if needed) the package cache rooted at `root`.
///
/// A free function rather than an inherent one: [`PackageCache`] is an alias
/// for a lock type that belongs to another crate.
pub fn open(root: PathBuf) -> Result<PackageCache, LockError> {
