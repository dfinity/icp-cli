...
/// The result of the fetch stage, as this resolver sees it.
struct FetchedTemplate {
    ...
    ///
    /// Committed by [`Resolve::commit`] once the template has rendered.
...
impl RecipeFetcher {
    ...
    async fn fetch_recipe(&self, recipe: &Recipe) -> Result<FetchedTemplate, RecipeFetchError> {
    ...
    /// Where a recipe's template belongs in the cache, or `None` for a recipe
    /// that is never cached because it is read from the project itself.
    fn cache_target(&self, recipe: &Recipe) -> Option<CacheTarget> {
        match &recipe.recipe_type {
            RecipeType::File(_) => None,
            RecipeType::Url(u) => Some(CacheTarget::Uri(u.clone())),
            RecipeType::Registry {
                name,
                recipe: recipe_name,
                version,
            } => Some(CacheTarget::Registry {
                package: format!("@{name}/{recipe_name}"),
...
impl Resolve for RecipeFetcher {
    async fn resolve(&self, recipe: &Recipe) -> Result<Fetched, ResolveError> {
        let fetched = self.fetch_recipe(recipe).await.map_err(ResolveError::new)?;
        Ok(Fetched {
            template: fetched.template,
            deferred: fetched.pending_cache.is_some(),
        })
    }

    /// The write `resolve` held back is rebuilt here from the recipe and the
    /// template that rendered, rather than carried across the trait: where a
    /// template belongs in the cache is this resolver's business, and the
    /// project layer has no use for a value it can only hand straight back.
    async fn commit(&self, recipe: &Recipe, fetched: &Fetched) -> Result<(), ResolveError> {
        let Some(target) = self.cache_target(recipe) else {
            return Ok(());
        };
        let pending = PendingCache {
            target,
            hash: Sha256::digest(fetched.template.as_bytes()).into(),
            template: fetched.template.clone(),
        };
        self.write_cache(&pending).await.map_err(ResolveError::new)
    }
}
...
mod tests {
    ...
    fn fetcher(cache_dir: &Path) -> RecipeFetcher {
        RecipeFetcher {
            ...
            pkg_cache: std::sync::Arc::new(crate::package::open(cache_dir.to_owned()).unwrap()),
        }
    }
