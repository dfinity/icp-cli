        ...
        Mode::Project(_) => {
            ...
            let icp_e8s_to_deposit = if let Some(icp_amount) = &args.icp {
                ...
                let cmc_response = agent
                    ...
                    .with_arg(
                        Encode!(&()).expect("Failed to encode get ICP XDR conversion rate args"),
                    )
                    .call()
                    .await
                ...
                let cmc_response = Decode!(&cmc_response, ConversionRateResponse)
                    .expect("CMC response type changed");
            ...
            let transfer_response = Decode!(&transfer_result, TransferResult)
                .expect("ICP ledger transfer result type changed");
            let block_index = match transfer_response {
                Ok(block_index) => block_index,
                Err(err) => match err {
                    TransferError::TxDuplicate { duplicate_of } => duplicate_of,
                    TransferError::InsufficientFunds { balance } => {
                        let required = BigDecimal::new(
                            (icp_e8s_to_deposit + ICP_LEDGER_BLOCK_FEE_E8S).into(),
                            8,
                        );
