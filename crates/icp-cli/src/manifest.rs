...
//!
//! The manifest *shapes* live in [`icp::manifest`]; reading them off disk is a
//! frontend concern, so the native [`ProjectRootLocate`] implementation and the
//! YAML loader live here.

use std::collections::HashSet;

use icp::{
    fs,
    manifest::{PROJECT_MANIFEST, ProjectRootLocate, ProjectRootLocateError},
    prelude::*,
};
...
impl ProjectRootLocate for ProjectRootLocateImpl {
    ...
    fn locate_member(&self) -> Result<PathBuf, ProjectRootLocateError> {
        ...
        nearest_manifest_dir(&self.cwd).ok_or_else(|| ProjectRootLocateError::NotFound {
