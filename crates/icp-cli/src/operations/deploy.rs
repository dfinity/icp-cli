...
//! event stream: each phase is a group task, and the per-canister tasks the
...
use icp_events::{Reporter, Task, TaskOutcome, TaskReporter};
...
pub(crate) struct DeployParams {
    ...
    pub(crate) init_args: Option<Vec<u8>>,
}
...
pub(crate) struct DeployReport {
    ...
    pub(crate) created: Vec<(String, Principal)>,
}
...
pub(crate) async fn deploy(
    ...
    params: &DeployParams,
    reporter: &Reporter,
) -> Result<DeployReport, anyhow::Error> {
    ...
    let phase = reporter.task(Task::group("Building canisters:"));
    ...
    let created = if canisters_to_create.is_empty() {
        reporter.notice("All canisters already exist");
        Vec::new()
        ...
        let phase = reporter.task(Task::group("Creating canisters:"));
    ...
    let phase = reporter.task(Task::group("Setting environment variables:"));
    ...
    let phase = reporter.task(Task::group("Applying canister settings:"));
    ...
    if !params.yes {
        let phase = reporter.task(Task::group("Checking Candid compatibility:"));
    ...
    // Install
    let phase = reporter.task(Task::group("Installing canisters:"));
...
async fn create_canisters(
    ...
    reporter: &Reporter,
) -> Result<Vec<(String, Principal)>, anyhow::Error> {
    ...
    for name in canisters_to_create.iter() {
        ...
        futs.push_back(async move {
            ...
            match &result {
                ...
                // The command's returned error reports this; a deferred dump
...
async fn sync(
    ...
    if sync_canisters.is_empty() {
        reporter.notice("No canisters have sync steps configured");
        return Ok(());
    }
    ...
    let phase = reporter.task(Task::group("Syncing canisters:"));
...
/// Close a phase's group task from its result, and hand the result back.
///
/// The phase heading carries no failure text of its own: whichever child
/// failed already said what went wrong, and the error itself is on the return
/// path.
fn finish<T, E: std::fmt::Display>(phase: &TaskReporter, result: Result<T, E>) -> Result<T, E> {
    match &result {
        Ok(_) => phase.finish(TaskOutcome::succeeded()),
        Err(error) => phase.finish(TaskOutcome::failed_silently(error.to_string())),
    }
    result
}
