...
/// Drives every `--*-status-viewer` / `--status-visibility` flag against a live
...
#[tokio::test]
async fn canister_settings_status_visibility() {
    let ctx = TestContext::new();
    ...
    // --add-status-viewer grants it to her, and the full report is printed.
    update(
        &ctx,
        &project_dir,
    ...
    // Add and remove in one call, relative to the current list. Alice loses
    // access again, so the fallback comes back.
    update(
        &ctx,
        &project_dir,
        &[
...
async fn canister_settings_sync_status_visibility() {
    ...
    .expect("failed to write project manifest");
    sync(&ctx, &project_dir);
    confirm(&ctx, &project_dir).stdout(contains(
        "Status visibility: Allowed viewers: 2vxsx-fae, aaaaa-aa",
    ));

    // Both visibility settings can be driven from the manifest at once, and a
    // single sync has to apply both — each change alone would satisfy the
    // "settings already match" check that decides whether to send an update.
    write_string(
        &project_dir.join("icp.yaml"),
        &manifest(
    ...
    sync(&ctx, &project_dir);
    confirm(&ctx, &project_dir)
        .stdout(contains("Status visibility: Controllers").and(contains("Log visibility: Public")));
}
