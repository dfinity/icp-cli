...
use crate::common::{
    ENVIRONMENT_COMPOSE, ENVIRONMENT_DOCKER, ENVIRONMENT_RANDOM_PORT, NETWORK_COMPOSE,
    NETWORK_DOCKER, NETWORK_RANDOM_PORT, TestContext, TestNetwork, clients,
};
use icp::{
...
#[tag(docker)]
#[tokio::test]
async fn network_compose() {
    let ctx = TestContext::new();

    let project_dir = ctx.create_project_dir("compose-network");

    // Copy compose file to project
    ctx.copy_asset_dir("compose", &project_dir.join("compose"));
    ...
    ctx.docker_pull_network();
    let _guard = ctx.start_network_in(&project_dir, "compose-network").await;
    ctx.ping_until_healthy(&project_dir, "compose-network");

    // Verify the network descriptor has compose child locator
    let descriptor_contents = ctx.read_network_descriptor(&project_dir, "compose-network");
    let descriptor: serde_json::Value =
        serde_json::from_slice(&descriptor_contents).expect("descriptor should be valid JSON");

    let child_locator = descriptor
        .get("child-locator")
        .expect("descriptor should have child-locator");

    // Check compose-specific fields
    assert!(
        child_locator.get("project-name").is_some(),
        "compose child locator should have project-name"
    );
    assert!(
        child_locator.get("compose-file").is_some(),
        "compose child locator should have compose-file"
    );
    assert!(
        child_locator.get("socket").is_some(),
        "compose child locator should have socket"
    );

    // Verify the project name follows the convention
    let project_name = child_locator
        .get("project-name")
        .and_then(|p| p.as_str())
        .expect("project-name should be a string");
    assert_eq!(
        project_name, "icp-compose-network",
        "compose project name should be icp-<network-name>"
    );

    // Verify network is healthy by checking balance
    let balance = clients::ledger(&ctx)
        .balance_of(Principal::anonymous(), None)
        .await;
    assert!(balance > 0_u128);
...
#[tag(docker)]
#[tokio::test]
async fn network_compose_stop() {
    let ctx = TestContext::new();

    let project_dir = ctx.create_project_dir("compose-stop");
    ...
    ctx.icp()
        .current_dir(&project_dir)
        .args(["network", "start", "compose-network", "--background"])
        .assert()
        .success();

    let network = ctx.wait_for_network_descriptor(&project_dir, "compose-network");
    ctx.ping_until_healthy(&project_dir, "compose-network");

    // Verify descriptor file exists
    let descriptor_file = project_dir
        .join(".icp")
        .join("cache")
        .join("networks")
        .join("compose-network")
        .join("descriptor.json");
    assert!(
        descriptor_file.exists(),
        "descriptor file should exist before stop"
    );

    // Stop the network
    ctx.icp()
        .current_dir(&project_dir)
        .args(["network", "stop", "compose-network"])
        .assert()
        .success()
        .stdout(contains(
            "Stopping compose network (project: icp-compose-network)",
        ))
    ...
    assert!(
        !descriptor_file.exists(),
        "descriptor file should be removed after stop"
    );
...
#[tokio::test]
async fn network_compose_file_not_found() {
    let ctx = TestContext::new();

    let project_dir = ctx.create_project_dir("compose-missing");

    // Project manifest without copying the compose file
    write_string(
        &project_dir.join("icp.yaml"),
        &formatdoc! {r#"
            {NETWORK_COMPOSE}
    ...
    // Start should fail with file not found error
    ctx.icp()
        .current_dir(&project_dir)
        .args(["network", "start", "compose-network", "--background"])
        .assert()
        .failure()
        .stderr(contains("docker compose file not found"));
}
