...
/// Test that binding to a non-localhost loopback IP (127.0.0.2) works end-to-end:
/// the gateway binds on the right address, deploy prints `?canisterId=` URLs
/// instead of broken subdomains of a bare IP, canister status works, and
/// assets are fetchable via the query-parameter URL form.
///
/// Only runs on Linux — macOS doesn't route 127.0.0.2 by default.
#[cfg(target_os = "linux")]
#[tokio::test]
async fn network_non_localhost_loopback_bind() {
    use icp::{fs::create_dir_all, store_id::IdMapping};

    let ctx = TestContext::new();
    let project_dir = ctx.create_project_dir("loopback-bind");
    ...
    let _guard = ctx.start_network_in(&project_dir, "loopback-network").await;
    ctx.ping_until_healthy(&project_dir, "loopback-network");

    // Read the raw descriptor to verify the host field
    let descriptor: Value =
        serde_json::from_slice(&ctx.read_network_descriptor(&project_dir, "loopback-network"))
            .expect("failed to parse descriptor");
    let host = descriptor["gateway"]["host"]
        .as_str()
        .expect("descriptor missing gateway.host");
    assert_eq!(host, "127.0.0.2", "descriptor host should be the bare IP");
    let port = descriptor["gateway"]["port"].as_u64().unwrap() as u16;

    // Deploy: URLs should use ?canisterId= form, not a broken subdomain of an IP
    clients::icp(&ctx, &project_dir, Some("loopback-env".to_string())).mint_cycles(10 * TRILLION);
    ...
    ctx.icp()
        ...
        .assert()
        .success()
        .stdout(contains("?canisterId="))
        .stdout(contains("Deployed canisters:"));

    // Read the canister ID
    let id_mapping: IdMapping = icp::fs::json::load(
        &project_dir
            .join(".icp")
            .join("cache")
            .join("mappings")
            .join("loopback-env.ids.json"),
    )
    .expect("failed to read ID mapping");
    let cid = id_mapping
        .get("my-assets")
        .expect("canister ID not found for my-assets");

    // canister status should work via the descriptor's host
    ctx.icp()
        .current_dir(&project_dir)
        .args([
            "canister",
    ...
    // Fetch the asset over the non-localhost loopback IP
    let resp = reqwest::get(format!("http://127.0.0.2:{port}/?canisterId={cid}"))
        .await
        .expect("request to 127.0.0.2 failed");
