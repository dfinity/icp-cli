...
async fn sync_aborts_when_canister_not_running() {
    ...
    // `DEBUG icp::events: syncing` marker rather than the bare word "syncing".)
    ctx.icp()
        .current_dir(&project_dir)
        .env("NO_COLOR", "1")
        .args([
        ...
        .stderr(
            contains("asset sync requires it to be Running")
                ...
                .and(contains("DEBUG icp::events: syncing").not()),
        );
}
...
async fn sync_multiple_canisters() {
    ...
    ctx.icp()
        ...
        .stderr(contains("DEBUG icp::events: syncing canister-a"))
        .stderr(contains("DEBUG icp::events: syncing canister-b"))
        .stderr(contains("DEBUG icp::events: syncing canister-c").not());
}
...
async fn sync_all_canisters_in_environment() {
    ...
    ctx.icp()
        ...
        .stderr(contains("DEBUG icp::events: syncing canister-a"))
        .stderr(contains("DEBUG icp::events: syncing canister-b"))
        .stderr(contains("DEBUG icp::events: syncing canister-c").not()); // not in test-env
}
