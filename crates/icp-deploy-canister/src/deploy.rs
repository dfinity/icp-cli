...
pub enum SyncStepError {
    #[snafu(transparent)]
    ResolvePlugin {
        source: crate::sync_exec::UnknownCallableCanisterError,
    },
...
/// Run a canister's configured sync steps through the injected executors,
...
pub async fn run_sync_steps(
    ...
    for step in &canister.sync.steps {
        // This crate owns dispatch and input derivation; the executors only run
        // the fully-resolved invocation.
        let step_lines = match step {
            SyncStep::Plugin(adapter) => match PluginInvocation::new(adapter, ctx) {
                Ok(invocation) => plugin_exec
                    .run_plugin(invocation, progress)
                    .await
                    .map_err(SyncStepError::from),
                Err(err) => Err(SyncStepError::from(err)),
            },
...
pub async fn sync_canister(
    ...
    run_sync_steps(canister, ctx, plugin_exec, script_runner, progress).await
}
...
pub async fn deploy_canister(
    ...
    let args = match manifest_args {
        Some((field, a)) => Some(a.to_bytes().context(InstallArgsSnafu {
            canister: canister_name,
            field,
        })?),
        None => None,
    ...
    let lines = run_sync_steps(canister, &ctx, plugin_exec, script_runner, progress).await?;
    Ok(lines)
}
...
mod tests {
    ...
    /// and mock sync executors, plus the raw args of each management call.
    #[derive(Default)]
    struct Log {
        events: Vec<String>,
    ...
    #[cfg_attr(target_family = "wasm", async_trait(?Send))]
    #[cfg_attr(not(target_family = "wasm"), async_trait)]
    impl ScriptRunner for MockExec {
        async fn run_script(
            &self,
            _invocation: ScriptInvocation,
            _progress: Option<&dyn StepProgress>,
        ) -> Result<Vec<String>, crate::sync_exec::ScriptRunError> {
            self.log
                .lock()
                .unwrap()
                .events
    ...
    fn ctx_for(canister_id: Principal) -> SyncStepContext {
        SyncStepContext {
            ...
            urls: crate::testutil::network_urls(),
    ...
    async fn sync_canister_applies_env_vars_before_steps() {
        ...
        sync_canister(&canister, cid, &ctx, &icp, &exec, &exec, None)
            .await
            .unwrap();
    ...
    fn unresolved_binding_is_skipped() {
        let canister = canister_with_binding();
        let vars = binding_env_vars(&canister, &BTreeMap::new());
        let names: Vec<&str> = vars.iter().map(|v| v.name.as_str()).collect();
        assert_eq!(names, ["FOO"], "unresolved bindings must not be emitted");
    }
}
