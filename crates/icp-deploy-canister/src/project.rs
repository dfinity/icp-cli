...
async fn resolve_manifest_settings(
    ...
    if let Some(vars) = environment_variables {
        ...
        for (name, var) in vars {
            let value = match var {
                ...
                ManifestEnvVar::Path { path } => {
                    ...
                    env_files.insert(name.to_owned(), file);
                    contents.trim().to_owned()
                }
            };
            resolved.insert(name.to_owned(), value);
        }
        resolved_vars = Some(resolved);
    }
    ...
    let settings = Settings {
        ...
        environment_variables: resolved_vars,
        controllers: controllers.clone(),
    };
    Ok((settings, env_files))
}
            ...
            let (build, sync) =
                match &m.instructions {
                    ...
                    Instructions::Recipe {
                        ...
                        let fetched = recipe_resolver.resolve_recipe(recipe).await.context(
                            FetchRecipeSnafu {
                                recipe_type: recipe.recipe_type.clone(),
                            },
                        )?;
                        let steps = recipe::render_recipe(&fetched.template, recipe, &ctx)
                            .context(RenderRecipeSnafu {
                                recipe_type: recipe.recipe_type.clone(),
                            })?;
                        // The template rendered, so an unpinned download is now
                        // known good and safe to cache. Committing only here is what
                        // keeps a bad remote response from becoming sticky.
                        recipe_resolver
...
pub fn relative_prefix(app_root_canonical: &Path, dep_canonical: &Path) -> String {
