...
pub struct PluginInvocation {
    ...
    /// The project directory: the sandbox boundary. A declared path may rise out
    /// of `base_dir` with `..` and reach anything inside the project, but
    /// nothing above it.
    pub project_dir: PathBuf,
    ...
    pub environment: String,
    /// The network's endpoints, exposed to the plugin via its `SyncExecInput`.
    pub urls: NetworkUrls,
...
/// directly in the app root, so the syncing canister's namespace is the prefix
/// of its own key.
///
...
fn exposed_canister_ids(ctx: &SyncStepContext) -> BTreeMap<String, Principal> {
    ...
    let Some((syncing_namespace, _)) = ctx.canister_name.rsplit_once(':') else {
        return ctx.canister_ids.clone();
    };
...
/// working directory and the environment the subprocess runs with (see
/// [`system_env_vars`]); the host only spawns the command(s).
...
pub struct ScriptInvocation {
    ...
    /// insertion order — not the complete environment. A host runner overlays
...
mod tests {
    ...
    use crate::testutil::network_urls;
    ...
    fn base_env_vars_are_always_set() {
        let ctx = ctx_named("backend", &[]);
        assert_eq!(
            system_env_vars(&ctx),
            vec![
                ("ICP_CLI_ENVIRONMENT".to_owned(), "demo".to_owned()),
                ("ICP_CLI_NETWORK".to_owned(), "ic".to_owned()),
                ("ICP_CLI_CID".to_owned(), principal(0).to_text()),
            ]
        );
    }
    ...
    fn canister_names_are_normalized_into_env_var_keys() {
        let ctx = ctx_named(
            "backend",
            &[("my-frontend", principal(2)), ("dep:api", principal(3))],
        );
        let keys: Vec<String> = system_env_vars(&ctx)
            .into_iter()
            .map(|(k, _)| k)
