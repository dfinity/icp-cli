...
mod tests {
    use super::*;
    use crate::network::{Gateway, Port, SubnetKind};

    fn default_config() -> ManagedLauncherConfig {
    ...
    #[test]
    fn flags_default_config_empty() {
        let config = default_config();
        assert!(launcher_settings_flags(&config).is_empty());
    }
    ...
    fn flags_ii() {
        ...
        assert_eq!(launcher_settings_flags(&config), vec!["--ii"]);
    }
    ...
    fn flags_nns() {
        ...
        assert_eq!(launcher_settings_flags(&config), vec!["--nns"]);
    }

    #[test]
    fn flags_artificial_delay() {
        let config = ManagedLauncherConfig {
            artificial_delay_ms: Some(100),
            ..default_config()
        };
        assert_eq!(
            launcher_settings_flags(&config),
            vec!["--artificial-delay-ms=100"]
        );
    }
    ...
    fn flags_subnets() {
        let config = ManagedLauncherConfig {
            subnets: Some(vec![SubnetKind::Application, SubnetKind::Bitcoin]),
            ..default_config()
        };
        assert_eq!(
            launcher_settings_flags(&config),
            vec!["--subnet=application", "--subnet=bitcoin"]
        );
    }

    #[test]
    fn flags_bitcoind_addr() {
        let config = ManagedLauncherConfig {
        ...
        assert_eq!(
            launcher_settings_flags(&config),
            vec!["--bitcoind-addr=127.0.0.1:18444"]
        );
    }

    #[test]
    fn flags_dogecoind_addr() {
        let config = ManagedLauncherConfig {
        ...
        assert_eq!(
            launcher_settings_flags(&config),
            vec!["--dogecoind-addr=127.0.0.1:22556"]
        );
    }

    #[test]
    fn flags_full_config() {
        let config = ManagedLauncherConfig {
            gateway: Gateway {
            ...
            ii: true,
            nns: true,
            subnets: Some(vec![SubnetKind::Application]),
            bitcoind_addr: Some(vec!["127.0.0.1:18444".to_string()]),
            dogecoind_addr: Some(vec!["127.0.0.1:22556".to_string()]),
        };
        let flags = launcher_settings_flags(&config);
        assert_eq!(
            flags,
            vec![
                "--ii",
                "--nns",
                "--artificial-delay-ms=50",
                "--subnet=application",
