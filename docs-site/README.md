...
- **`robots.txt`** — generated dynamically by the CI `publish-root-files` job from `versions.json`; allows only the latest version's path and disallows old versions and `/main/`. Never placed in versioned build output.
- **`sitemap.xml`** — root sitemap index delegating to the latest version's `sitemap-index.xml`; also generated by `publish-root-files`.
