...
A dependency's own same-named environment still contributes its **canister selection**, **per-canister settings**, and **init args**, so a vendored project's canisters get the configuration their author intended. Precedence, highest first:
