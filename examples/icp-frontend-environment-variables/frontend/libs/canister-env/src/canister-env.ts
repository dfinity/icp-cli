...
export function getCanisterEnv<T = Record<string, never>>(
  ...
  if (!cookieValue) {
    throw new Error(`Cookie ${cookieName} is empty`);
  }

  const decodedCookieValue = decodeURIComponent(cookieValue);
  const envVars = parseEnvVars<T>(decodedCookieValue);
  if (!envVars.IC_ROOT_KEY) {
    throw new Error(`Root key not found in cookie ${cookieName}`);
  }
...
function parseEnvVars<T = Record<string, never>>(
  decoded: string
): CanisterEnv & T {
  const entries = decoded.split(ENV_VAR_SEPARATOR).map((v) => {
    ...
    if (key === IC_ROOT_KEY_VALUE_NAME) {
      ...
      if (rootKey.length !== IC_ROOT_KEY_BYTES_LENGTH) {
        throw new Error(
          `Invalid root key length: ${rootKey.length}, expected ${IC_ROOT_KEY_BYTES_LENGTH}`
        );
      }
