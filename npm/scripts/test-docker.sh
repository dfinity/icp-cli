...
# Test icp-cli npm package using Docker
...
echo "=========================================="
echo "icp-cli Docker Testing Suite"
echo "=========================================="
echo ""
...
case $TEST_TYPE in
    quick)
        ...
        echo ""
        docker build -f Dockerfile.test -t icp-cli-test:latest .
        docker run --rm icp-cli-test:latest
        ;;
    ...
    interactive)
        ...
        echo ""
        docker build -f Dockerfile.test -t icp-cli-test:latest .
        docker run --rm -it icp-cli-test:latest /bin/bash
        ;;
    
    clean)
        echo "Cleaning up Docker images..."
        docker rmi icp-cli-test:latest 2>/dev/null || true
        if command -v docker-compose &> /dev/null || docker compose version &> /dev/null 2>&1; then
            if docker compose version &> /dev/null 2>&1; then
